...
local function MakeWallType2(data)
    local assets =
    {
        --Asset("ANIM", "anim/wall.zip"),
        Asset("ANIM", "anim/wall_"..data.name..".zip"),
		Asset("IMAGE", "images/inventoryimages/wall_"..data.name..".tex"),
		Asset("ATLAS", "images/inventoryimages/wall_"..data.name..".xml"),
    }
    ...
    local function itemfn()
		...
		inst:AddComponent("ccs_wall")
        inst:AddComponent("inspectable")
        inst:AddComponent("inventoryitem")
		inst.components.inventoryitem.atlasname = "images/inventoryimages/wall_"..data.name..".xml"
		inst.components.inventoryitem.imagename = "wall_"..data.name
        ...
        inst.components.repairer.repairmaterial = "wall_"..data.name.."_item"
    ...
    local function fn()
        ...
        inst:AddTag("wall")
		inst:AddTag("ccs_wall")
        inst:AddTag("noauradamage")

        inst.AnimState:SetBank("wall_"..data.name)
        inst.AnimState:SetBuild("wall_"..data.name)
        ...
        inst.components.repairable.repairmaterial = "wall_"..data.name.."_item"
    ...
    return Prefab("wall_"..data.name, fn, assets, prefabs),
        ...
        MakePlacer("wall_"..data.name.."_item_placer", "wall", "wall_"..data.name, "onequarter", false, false, true, nil, nil, "eight")
end
...
local walldata =
{
    { name = "ccs_wall1",     material = "stone",  tags = { "stone" },  loot = "marble",   maxloots = 2, maxhealth = 500,     flammable = true, buildsound = "dontstarve/common/place_structure_wood"  },
}
for i, v in ipairs(walldata) do
    local wall, item, placer = MakeWallType2(v)
