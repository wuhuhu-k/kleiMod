--------------------------------------------------------------------------
local ANIM_XIAOYING_LEAF_TEXTURE = resolvefilepath("images/fx/xiaoying_leaf.tex") -- 这里一定要用 resolvefilepath() 包装一下，否则会出现找不到贴图的错误
local function IntColour(r, g, b, a)
    return { r / 255, g / 255, b / 255, a / 255 }
end

--local TEXTURE = "fx/xiaoying_leaf.tex"
...
local assets =
{
    Asset("IMAGE", ANIM_XIAOYING_LEAF_TEXTURE),
    Asset("SHADER", SHADER),
}
...
local function InitEnvelope()
    ...
    EnvelopeManager:AddColourEnvelope(COLOUR_ENVELOPE_NAME..0,
        {
            { 0, IntColour(255, 99, 0, 255) },
            { 0.5, IntColour(255, 99, 0, 255) },
            { 1, IntColour(255, 0,99, 0) },
        }
    )
...
local function fn()
    ...
    for i=0,num_emitters do
        effect:SetRenderResources(i, ANIM_XIAOYING_LEAF_TEXTURE, SHADER)
...
return Prefab("xiaoying_leaf_fx", fn, assets)
