...
local ui = Class(Widget, function(self, owner,container, name)
    ...
    self.bg:SetScale(0.25, 0.30)
    self.bg:SetPosition(80, 20)
    ...
    local btn = self:AddChild(ImageButton())
    btn:SetText("开关全局制作模式")
    btn:SetPosition(80, -60, 0)
    btn:ForceImageSize(270, 70)
    btn:SetOnClick(function ()
        RPC:PushEvent("Sora2ChestControl",{cmd="GlobalBuild"})
    end)
