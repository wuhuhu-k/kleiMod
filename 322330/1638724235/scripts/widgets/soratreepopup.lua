...
local ui = Class(Widget, function(self)
    Widget._ctor(self, "SoraTree  Popup")
    
    self.bg = self:AddChild(Image("images/hud.xml","craftingsubmenu_fullhorizontal.tex"))
    self.bg:SetPosition(630,-90,0)
    self.bg:SetScale(1.75, 1.75, 1.75)

    self.contents = self:AddChild(Widget("contents"))
    self.contents:SetPosition(630,-90 ,0)
