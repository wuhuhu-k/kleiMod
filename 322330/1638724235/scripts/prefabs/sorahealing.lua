...
]] local assets = {Asset("ANIM", "anim/sorahealing.zip"), Asset("ATLAS", "images/inventoryimages/sorahealing.xml"),
...
local function OnRefuseItem(inst, giver, item)
    if item then
        local refusesay = "\t\t穹の愈\n物\t数\t级\t属"
        refusesay = refusesay ..
                        string.format("\n攻击(紫):\t%d/%d\t%d\t%d", inst.zbsnum, inst.need * inst.maxlevel,
                inst.zbslevel, math.max(att1 + inst.zbslevel * att2, 0))
        refusesay = refusesay ..
                        string.format("\n冷却(黄):\t%d/%d\t%d\t%d", inst.hbsnum, inst.need * inst.maxlevel,
                inst.hbslevel, math.max(cd1 - inst.hbslevel * cd2, 0))
        refusesay = refusesay ..
                        string.format("\n治疗(绿):\t%d/%d\t%d\t%d", inst.lbsnum, inst.need * inst.maxlevel,
                inst.lbslevel, 20 + 3 * inst.lbslevel)
    ...
    if (item.prefab == "purplegem") then
        ...
        if inst.zbslevel < inst.maxlevel then
            giver.components.talker:Say(
                "紫宝石数量:" .. inst.zbsnum .. "/" .. inst.need * inst.maxlevel .. "\tLV:" .. inst.zbslevel ..
                    "\n攻击力：" .. (math.max(att1 + inst.zbslevel * att2, 0)))

        else
            giver.components.talker:Say("紫宝石已满\tLV:" .. inst.maxlevel .. "\n攻击力：" ..
                                            math.max(att1 + inst.zbslevel * att2, 0))
        end
    elseif (item.prefab == "yellowgem") then
        ...
        if inst.hbslevel < inst.maxlevel then
            giver.components.talker:Say(
                "黄宝石数量:" .. inst.hbsnum .. "/" .. inst.need * inst.maxlevel .. "\tLV:" .. inst.hbslevel ..
                    "\n冷却时间：" .. (math.max(cd1 - inst.hbslevel * cd2, 0)))

        else
            giver.components.talker:Say("黄宝石已满\tLV:" .. inst.maxlevel .. "\n冷却时间：" ..
                                            math.max(cd1 - inst.hbslevel * cd2, 0))
        end
    elseif (item.prefab == "greengem") then
        ...
        if inst.lbslevel < inst.maxlevel then
            giver.components.talker:Say(
                "绿宝石数量:" .. inst.lbsnum .. "/" .. inst.need * inst.maxlevel .. "\tLV:" .. inst.lbslevel ..
                    "\n生命回复：" .. math.max(player1 + inst.lbslevel * player2, 0))
        else
            giver.components.talker:Say("绿宝石已满\tLV:" .. inst.maxlevel .. "\n生命回复：" ..
                                            math.max(player1 + inst.lbslevel * player2, 0))
        end
    end
    upgrade(inst)
end
    ...
    if owner ~= nil and not owner:HasTag("sora") and false then
        owner:DoTaskInTime(0, function()
            ...
            if owner.components.talker then
                owner.components.talker:Say("这是Sora的法杖")
            end
        end)
        return
    end
...
local function onuse(inst)
    ...
    if inst.components.inventoryitem.owner then
        inst.components.inventoryitem.owner.components.talker:Say(inst.heal and
                                                                      "切换成功！现在会治疗怪物！" or
                                                                      "切换成功！现在会冰冻怪物！")
    end
    return false
...
local function soramagicfn(staff, target, pos)
    ...
    caster.components.hunger:DoDelta(-15 * TUNING.SORAMODE)
    local x, y, z = pos:Get()
