local assets = {
    Asset("ANIM", "anim/gentleness_lantern_post.zip"),
...
local prefabs = {
    "gentleness_lantern_light",
    "gentleness_lantern_light_chain",
}
...
local function IsLightOn(inst)
    return inst and inst.components.container and #inst.components.container:FindItems(is_battery_type) > 0 or false
end
...
--------------------------------------------------------------------------
local POST_MUST = { "gentleness_post" }
...
local function FindPartners(inst)
    ...
    local function spawnlight(inst, partner)
        ...
        local newlight = SpawnPrefab("gentleness_lantern_light_chain")
...
local function lightson(inst)
    ...
    if not inst.light_obj then
        local newlight = SpawnPrefab("gentleness_lantern_light")
...
local function fn()
    ...
    inst.AnimState:SetBuild("gentleness_lantern_post")
    inst.AnimState:PlayAnimation("idle", true)

    inst:AddTag("gentleness_post")

    inst.AnimState:Hide("light_on")

    inst.MiniMapEntity:SetIcon("gentleness_lantern_post_item.tex")

    inst.entity:SetPristine()

    if not TheWorld.ismastersim then
        inst["OnEntityReplicated"] = function(_inst)
            _inst["replica"]["container"]:WidgetSetup("gentleness_lantern_post")
        end
        return inst
    end
    ...
    inst.components.container:WidgetSetup("gentleness_lantern_post")
...
local function CreateChainLink(variation)
    ...
    inst.AnimState:SetBank("gentleness_lantern_post")
    inst.AnimState:SetBuild("gentleness_lantern_post")
...
local function replaceLantern(inst, index)
    if inst and inst["AnimState"] and type(index) == "number" then
        --inst["AnimState"]:OverrideSymbol("lantern", "gentleness_lantern", "lantern_" .. tostring(index))
    end
end
...
local function lightchainfn()
    ...
    inst.partner1 = net_entity(inst.GUID, "gentleness_lantern_light.partner1", "partner1dirty")
    inst.partner2 = net_entity(inst.GUID, "gentleness_lantern_light.partner2", "partner2dirty")
    inst.enablelight = net_tinybyte(inst.GUID, "gentleness_lantern_light.enablelight", "lightdirty")
...
return Prefab("gentleness_lantern_post", fn, assets, prefabs),
Prefab("gentleness_lantern_light", lightfn, assets, prefabs),
Prefab("gentleness_lantern_light_chain", lightchainfn, assets, prefabs),
MakeDeployableKitItem("gentleness_lantern_post_item", "gentleness_lantern_post", "gentleness_lantern_post", "gentleness_lantern_post", "kit",
        {
            Asset("ANIM", "anim/gentleness_lantern_post.zip"),
            Asset("IMAGE", "images/gentleness_image/gentleness_lantern_post_item.tex"),
            Asset("ATLAS", "images/gentleness_image/gentleness_lantern_post_item.xml"),
        },
        {
            size = "med", scale = 0.77
        }, nil,
        { fuelvalue = TUNING.LARGE_FUEL },
        deployable_data, TUNING.STACK_SIZE_MEDITEM, function(inst)
            if inst and inst["components"] and inst["components"]["inventoryitem"] then
                inst["components"]["inventoryitem"]["imagename"] = "gentleness_lantern_post_item"
                inst["components"]["inventoryitem"]["atlasname"] = "images/gentleness_image/gentleness_lantern_post_item.xml"
            end

        end),
MakePlacer("gentleness_lantern_post_item_placer", "gentleness_lantern_post", "gentleness_lantern_post", "placer", nil, nil, nil, nil, nil, nil, placer_postinit_fn)